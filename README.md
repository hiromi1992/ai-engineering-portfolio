...
**新規事業を約4年経験し、うち約2年はプロダクト／開発ディレクション・開発PMを担当**してきました。実務では、**課題設定 → プロダクト設計 → 開発 → リリース → データ分析 → 改善**まで経験しています。

現在はその経験をAI領域へ広げ、事業や業務の課題を整理し、**AI / Coding Agentを実装手段として、システムの要件定義・設計・検証・受入判断まで**を個人開発で実践しています。

コード生成・直接実装には主にCodex / ClaudeなどのCoding Agent（AI実装支援）を活用しています。検討過程ではAIを壁打ちに使いますが、**課題整理・要件定義・設計方針・実装指示・検証設計・レビュー・受入 / 却下・再設計は自分が決定**しています。コード生成・直接実装はCoding Agentへ委譲しています。

---

## 実務経験との接続

実務では、新規事業・Webプロダクトで、**課題整理・要件定義・画面 / 導線設計・開発推進・QA・リリース後のデータ確認と改善判断**まで担当してきました。

この公開Portfolioの役割は、**AI個人開発で何を設計し、何をAIへ委譲し、どのEvidenceを見て受入・却下・再設計したか**を示すことです。

- **Portfolioで示すもの**：技術的な設計判断、状態・権限・復旧、Evaluation、Evidence / Provenance、Holdout、Fail Closed、失敗からの再設計、実装・検証済み範囲
- **職務経歴書で示すもの**：実務での役割、開発体制・人数、事業成果、営業・顧客実績、プロダクトごとの担当範囲

そのため、この公開Portfolioでは**所属企業・プロダクト・取引先・非公開KPIなどの業務情報は扱わず**、実務成果を重複掲載しません。実務上の成果・役割の詳細は職務経歴書で提示します。

実務で培った**「リリース後の実データまで確認して次の判断をする」**考え方を、下記のAI個人開発にも応用しています。
...
コード生成・直接実装にはCoding Agentを活用しています。検討過程ではAIを壁打ちに使いますが、**要件・設計方針・テスト観点・レビュー・受入 / 却下は自分が決定**しています。
