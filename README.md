...
新規事業・開発PMとして、**課題設定 → Product設計 → 開発 → リリース → データ分析 → 改善**までを実務で経験してきました。
...
## 実務でProductを成果まで回した経験

### ヤギプライム — 送客機能の0→1立ち上げ

有料会員を人材紹介会社へ送客する新規機能・事業で、**サービス設計・導線設計・開発推進・QA・リリース・GA4分析・改善判断**までを担当しました。

- **2025年1〜3月：187名を送客**
- リリース後もファネルを継続分析
- UI変更で完了率が約16.3% → 10.1%へ低下したケースも検知し、Step別に離脱を分析して改善判断
- 別の導線改善では、LP利用者数が **750 → 1,966 UU（約2.6倍）**へ増加

→ **[実務Product Case Studyを見る](case-studies/04_business_product_delivery.md)**

この実務経験で培った**「リリース後の実データまで確認して次の判断をする」**考え方を、下記のAI個人開発にも応用しています。
...
- **[実務Product：送客機能を0→1で立ち上げ、187名を送客](case-studies/04_business_product_delivery.md)**  
  → Product設計からリリース後のGA4分析・改善判断までを担当
