...
新規事業・開発PMとして、**課題設定 → プロダクト設計 → 開発 → リリース → データ分析 → 改善**までを実務で経験してきました。
...
- **2025年1〜3月：187名を送客**
- リリース後もファネルを継続分析
- UI変更で完了率が約16.3% → 10.1%へ低下したケースも検知し、ステップ別に離脱を分析して改善判断
- 別の導線改善では、LP利用者数が **750 → 1,966 UU（約2.6倍）**へ増加

→ **[実務事例の詳細を見る](case-studies/04_business_product_delivery.md)**
...
- **[実務事例：送客機能を0→1で立ち上げ、187名を送客](case-studies/04_business_product_delivery.md)**  
  → プロダクト設計からリリース後のGA4分析・改善判断までを担当
